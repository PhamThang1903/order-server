...
def fetch_and_update(db: Session, order: Order) -> bool:
    ...
    try:
        ...
        with httpx.Client(timeout=15.0) as client:
            resp = client.get(order.tracking_url, headers=headers)
            resp.raise_for_status()
            data = resp.json()
    except (httpx.HTTPError, httpx.TimeoutException) as e:
        logger.error(f"Tracking HTTP error for order {order.order_code}: {e}")
        increment_retry(db, order)
        if order.tracking_retry_count >= 3:
            fcm_service.notify_tracking_error(db, order)
        return False
...
def increment_retry(db: Session, order: Order) -> None:
    order.tracking_retry_count += 1
    order.last_tracked_at = datetime.now(timezone.utc)
    db.commit()


def reset_retry(db: Session, order: Order) -> None:
